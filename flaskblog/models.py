...
class User(db.Model, UserMixin):
    ...
    # Check if password was used recently (last 5)
    def is_password_in_history(self, plain_password, history_limit=5):
        history = (
        ...
        for record in history:
            if bcrypt.check_password_hash(record.password_hash, plain_password):
                return True
        return False
...
class PasswordHistory(db.Model):
    ...
    user = db.relationship(
        'User',
        backref=db.backref('password_history', lazy='dynamic')
    )
